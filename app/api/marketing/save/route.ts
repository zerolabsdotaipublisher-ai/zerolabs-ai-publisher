...
import { storeWebsiteStructureContentSnapshot } from "@/lib/ai/content";
import { loadEditorStructure, saveEditorStructureDraft } from "@/lib/editor/storage";
import { getServerUser } from "@/lib/supabase/server";

interface SaveMarketingBody {
  structureId?: string;
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  let body: SaveMarketingBody;
  try {
    body = (await request.json()) as SaveMarketingBody;
  } catch {
  ...
  const content = await storeWebsiteStructureContentSnapshot(result.structure, user.id);
