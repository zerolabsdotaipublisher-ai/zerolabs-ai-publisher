...
import {
  ...
  parseRegenerationApplyPayload,
  runRegenerationApplyWorkflow,
...
export async function POST(
  ...
  const parsed = parseRegenerationApplyPayload(body);
  if (!parsed.payload) {
    return NextResponse.json({ ok: false, error: "Invalid apply request", validationErrors: parsed.validationErrors }, { status: 400 });
  }

  const result = await runRegenerationApplyWorkflow({
    userId: user.id,
    contentId,
    request: parsed.payload.request,
    regeneratedDraft: parsed.payload.regeneratedDraft,
  });
