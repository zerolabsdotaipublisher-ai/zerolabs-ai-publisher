...
import { normalizeEditingContentIdParam, parseEditableSavePayload, runOwnedEditingAutosaveWorkflow } from "@/lib/editing";
...
export async function POST(
  ...
  const result = await runOwnedEditingAutosaveWorkflow(user.id, payload.draft);
  if (!result.ok) {
  ...
  return NextResponse.json({ ok: true, detail: result.detail, autoSaved: result.autoSaved });
}
